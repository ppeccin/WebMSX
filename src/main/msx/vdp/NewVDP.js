...
wmsx.NewVDP = function(cpu, psg) {
    ...
    function updatePatternPlaneMode0(line, bufferPos) {                     // Graphics 1 (Screen 1)
    ...
    function updatePatternPlaneMode1(line, bufferPos) {                     // Graphics 2 (Screen 2)
    ...
    function updatePatternPlaneMode2(line, bufferPos) {                     // Multicolor (Screen 3)
    ...
    function updatePatternPlaneMode4(line, bufferPos) {                     // Text (Screen 0)
    ...
    function updateSpritePlanesSize2(line, bufferPos) {                     // 16x16 normal
        ...
        if ((status & 0x40) === 0) {                                        // Only set if 5S are still unset
            if (invalid >= 0) {
                ...
                if (sprite > (status & 0x1f)) status |= sprite;
        }
    }

    function updateSpritePlanesSize0(line, bufferPos) {                     // 8x8 normal
        if (vramSpriteAttrTable[0] === 208) return;                         // Sprites deactivated
    ...
    function updateSpritePlanesSize3(line, bufferPos) {                     // 16x16 double
        if (vramSpriteAttrTable[0] === 208) return;                         // Sprites deactivated
    ...
    function updateSpritePlanesSize1(line, bufferPos) {                     // 8x8 double
        if (vramSpriteAttrTable[0] === 208) return;                         // Sprites deactivated
