...
function VDP(cpu) {
    ...
    this.input98 = function(port) {
        // VRAM Read
        return vramAll[vramPointer++];
    };

    this.output98 = function(port, val) {
        // VRAM Write
        vramAll[vramPointer++] = val;
    };
    ...
    function initPlaneResources() {
        ...
        patternPlaneImageData = patternPlaneContext.getImageData(0, 0, patternPlaneCanvas.width, patternPlaneCanvas.height);
        if (patternPlaneImageData.bytes.buffer)
            patternPlaneBackBuffer = new Uint32Array(patternPlaneImageData.bytes.buffer);
        else {
            // Needed for IE compatibility, which has no underlying buffer
            patternPlaneBackData = patternPlaneImageData.bytes;
        }
    }
    ...
    this.textModeUpdater = null;
    this.updateTextMode = function () {
        if (!this.textModeUpdater) return;

        var res = "";
        if (register1 & 0x40) {
            for (var i = 0; i < 960; i++)
                res += String.fromCharCode(vramNameTable[i]);
        }

        this.texModeContent = res;
        this.textModeUpdater(res);
    };

    this.canvasUpdater = null;
    this.updateCanvas = function () {
        if (!this.canvasUpdater) return;

        if (register1 & 0x40) {
            //patternPlaneContext.putImageData(patternPlaneImageData, 0, 0);
            //this.canvasUpdater(patternPlaneCanvas);
            this.canvasUpdater(patternPlaneImageData);
        }
    };
